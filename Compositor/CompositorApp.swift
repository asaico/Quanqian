            ...
            .commands {
                CommandGroup(replacing: .undoRedo) {
                    ...
                    if session.levels != nil || session.isProjectBusy || session.showsNewDocument || session.showsImporter || session.renamingLayerID != nil || session.transformEdit?.persistent == true {
                        ...
                        Button(session.history.canUndo ? String(format: "Undo %@".localized, session.history.undoName.localized) : "Undo".localized) { session.undo() }
                            .keyboardShortcut("z").disabled(!session.canUndo)
                        Button(session.history.canRedo ? String(format: "Redo %@".localized, session.history.redoName.localized) : "Redo".localized) { session.redo() }
                            .keyboardShortcut("z", modifiers: [.command, .shift]).disabled(!session.canRedo)
                    }
                }
                CommandGroup(replacing: .newItem) {
                    Button("New Canvas…".localized) {
                    ...
                    Button("Open Project…".localized) {
                        applicationDelegate.showEditor?()
                        Task { await applicationDelegate.projects.open() }
                    }
                        .keyboardShortcut("o").disabled(!applicationDelegate.projects.canStart)
                    Button("Import Images…".localized) { session.showsImporter = true }
                        .disabled(session.levels != nil || session.showsBusy || session.isImporting || session.showsNewDocument)
                }
                CommandGroup(replacing: .saveItem) {
                    Button("Save".localized) { Task { await applicationDelegate.projects.save() } }
                        .keyboardShortcut("s").disabled(session.document == nil || !applicationDelegate.projects.canStart)
                    Button("Save As…".localized) { Task { await applicationDelegate.projects.save(asNew: true) } }
                        .keyboardShortcut("s", modifiers: [.command, .shift])
                        .disabled(session.document == nil || !applicationDelegate.projects.canStart)
                    Divider()
                    Button("Export PNG…".localized) { Task { await applicationDelegate.projects.exportPNG() } }
                        .keyboardShortcut("e", modifiers: [.command, .shift])
                        .disabled(session.document == nil || !applicationDelegate.projects.canStart)
                    Button("Export JPEG…".localized) { Task { await applicationDelegate.projects.exportJPEG() } }
                        .keyboardShortcut("s", modifiers: [.command, .option, .shift])
                        .disabled(session.document == nil || !applicationDelegate.projects.canStart)
                    Divider()
                    Button("Close Project".localized) {
                ...
                Group {
                    CommandGroup(after: .appInfo) {
                        Button("Check for Updates…".localized) { applicationDelegate.updater.checkForUpdates(nil) }
                    }
                    CommandGroup(after: .toolbar) {
                        Button("Fit Canvas".localized) { session.fit() }.keyboardShortcut("0").disabled(session.document == nil)
                        Button("Actual Pixels".localized) { session.zoom(to: 1) }.keyboardShortcut("1").disabled(session.document == nil)
                        Button("Zoom In".localized) { session.zoom(to: session.viewport.zoom * 1.25) }
                            .keyboardShortcut("=").disabled(session.document == nil)
                        Button("Zoom Out".localized) { session.zoom(to: session.viewport.zoom / 1.25) }
                            .keyboardShortcut("-").disabled(session.document == nil)
                        Toggle("Pixel Grid (800% and above)".localized, isOn: Binding(get: { session.showsPixelGrid },
                                                                                      set: { session.showsPixelGrid = $0 }))
                        Toggle("Show Transform Controls".localized, isOn: Binding(get: { session.showsTransformControls },
                    ...
                    CommandGroup(replacing: .appVisibility) {
                        Button("Hide Compositor".localized) { NSApp.hide(nil) }
                        Button("Hide Others".localized) { NSApp.hideOtherApplications(nil) }
                ...
                CommandMenu("Select".localized) {
                    ...
                    Button("Mask's Black Areas".localized) {
                    ...
                    Divider()
                    Button(String(format: "Expand by %d px".localized, session.selectionExpandAmount)) { session.expandSelection(by: session.selectionExpandAmount) }
                        .disabled(!session.canModifySelection)
                    Button(String(format: "Contract by %d px".localized, session.selectionContractAmount)) { session.contractSelection(by: session.selectionContractAmount) }
                        .disabled(!session.canModifySelection)
                }
                CommandMenu("Image".localized) {
                    Button("Curves…".localized) { session.beginFilter(.curves) }
                        .keyboardShortcut("m").disabled(!session.canAdjustColors || session.hueSaturation != nil)
                    Button("Levels…".localized) { session.beginLevels() }
                        .keyboardShortcut("l").disabled(!session.canAdjustColors || session.hueSaturation != nil)
                    Button("Hue/Saturation…".localized) { session.beginHueSaturation() }
                    ...
                    Divider()
                    Button("Canvas Size…".localized) { Task { await applicationDelegate.projects.canvasSize() } }
                        .keyboardShortcut("c", modifiers: [.command, .option])
                        .disabled(session.document == nil || !applicationDelegate.projects.canStart)
                    Button("Image Size…".localized) { Task { await applicationDelegate.projects.imageSize() } }
                        .keyboardShortcut("i", modifiers: [.command, .option])
                        .disabled(session.document == nil || !applicationDelegate.projects.canStart)
                    Group {
                        Divider()
                        Button("Flip Canvas Horizontal".localized) { session.flipCanvas(horizontally: true) }
                            .disabled(!session.canEditLayers)
                        Button("Flip Canvas Vertical".localized) { session.flipCanvas(horizontally: false) }
                            .disabled(!session.canEditLayers)
                    }
                }
                CommandMenu("Filter".localized) {
