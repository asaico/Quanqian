...
final class ProjectController {
    ...
    func exportPNG() async {
        ...
        catch { await showError("Couldn’t export PNG".localized, error: error) }
    }
    ...
    func canvasSize() async {
        ...
        do {
            ...
            session.applyDocumentSize(resized, actionName: "Canvas Size".localized)
        } catch { await showError("Couldn’t change canvas size".localized, error: error) }
    }
    ...
    func imageSize() async {
        ...
        } catch { await showError("Couldn’t resize the image".localized, error: error) }
    }
    ...
    func exportJPEG() async {
        ...
        } catch { await showError("Couldn’t export JPEG".localized, error: error) }
    }
    ...
    private func saveCurrent(asNew: Bool = false) async -> Bool {
        ...
        if destination == nil {
            ...
            panel.canCreateDirectories = true
            panel.isExtensionHidden = false
            panel.nameFieldStringValue = session.projectURL?.lastPathComponent ?? ("Untitled".localized + ".comp")
        ...
        do {
            ...
            await showError("Couldn’t save the project".localized, error: error)
            return false
        }
    }
    ...
    func open(_ suppliedURL: URL? = nil) async -> Bool {
        ...
        do {
            ...
            await showError("Couldn’t open the project".localized, error: error)
            return false
        }
    }
    ...
    private func confirmReplacement() async -> Bool {
        ...
        let alert = NSAlert()
        alert.messageText = String(format: "Save changes to %@?".localized, session.projectURL?.lastPathComponent ?? "Untitled".localized)
    ...
    private func drainIncoming() async {
        while !incoming.isEmpty {
            ...
            if projects.count > 1 {
                await showError("Open one project at a time".localized, error: ProjectError.invalid)
            } else {
                var proceed = true
