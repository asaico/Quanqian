...
struct CurvesControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Picker("Channel".localized, selection: $settings.channel) {
            ...
            Text("Click to add a point. Drag to adjust.".localized).font(.caption).foregroundStyle(.secondary)
            HStack {
                if let selected, points.indices.contains(selected) {
                    Text(String(format: "Input %d · Output %d".localized, Int(points[selected].x), Int(points[selected].y))).monospacedDigit()
                }
                Spacer()
                Button("Remove point".localized) {
            ...
            Button("Reset curve".localized) { settings.channels[settings.channel.index] = [CurvePoint(x: 0,y: 0), CurvePoint(x: 255,y: 255)]; selected = nil }
        }
    }
}
