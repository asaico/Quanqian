    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            if session.tool == .brush {
                Picker("Mode".localized, selection: $session.brushMode) {
                    ForEach(BrushToolMode.allCases, id: \.self) { Text($0.rawValue.localized).tag($0) }
                }
                .pickerStyle(.segmented).labelsHidden().fixedSize()
                .help("Paint with the foreground color (B), or erase pixels away (E)".localized)
            }
            if session.tool == .blur {
                Picker("Mode".localized, selection: $session.blurMode) {
                    ForEach(BlurToolMode.allCases, id: \.self) { Text($0.rawValue.localized).tag($0) }
                }
                .pickerStyle(.segmented).labelsHidden().fixedSize()
                .help("Liquify pushes pixels · Blur softens · Smudge drags color along".localized)
            }
            if session.tool == .spotHealing {
                Picker("Type".localized, selection: $session.spotHealingMode) {
            ...
            if session.tool == .cloneStamp {
                Toggle("Aligned".localized, isOn: $session.cloneSettings.aligned)
                    .help("Keep the source moving with the brush between strokes; off starts every stroke at the source point".localized)
                Picker("Sample".localized, selection: $session.cloneSettings.sampleAllLayers) {
                    Text("This Layer".localized).tag(false)
                    Text("All Layers".localized).tag(true)
                }
                .pickerStyle(.segmented).labelsHidden().fixedSize()
                .help("Copy from the active layer only, or from every visible layer as shown".localized)
            }
            Text("Size".localized)
            TextField("Size".localized, value: Binding<Double>(get: { Double(session.brushSettings.diameter) },
                ...
                .unitSuffix("px")
            Text("Hardness".localized)
            Slider(value: $session.brushSettings.hardness, in: 0...1).frame(width: 100)
            TextField("Hardness".localized, value: Binding<Double>(get: { Double(session.brushSettings.hardness * 100) },
            ...
            TextField("Opacity".localized, value: Binding<Double>(get: { Double(session.brushSettings.opacity * 100) },
            ...
            if session.isMaskSelected {
                Picker("Paint".localized, selection: $session.maskPaintWhite) {
                    Text("Black · Hide".localized).tag(false)
                    Text("White · Reveal".localized).tag(true)
                }.frame(width: 180)
                ...
                HStack(spacing: 6) {
                    ...
                    .help("Foreground color".localized)
                    .accessibilityLabel("Foreground color".localized)
                }
            }
            Spacer(minLength: 0)
            if session.tool == .cloneStamp, session.cloneSource == nil {
                Text("Option-click to set the source".localized).foregroundStyle(.secondary)
            }
            if session.isMaskSelected { Text("Mask".localized).foregroundStyle(.secondary) }
        }
